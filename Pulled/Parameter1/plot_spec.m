...
CS = CH.^(-1/2);
figure(23)
plot(CS(40:end), TRIG(40:end),'LineWidth',3)
...
text(kap(1)-0.05,q(1)-0.01,'$-i k_\mathrm{tf}$','Interpreter','latex','FontSize',16)
...
annotation('arrow', [0.4, 0.39],[0.595,0.58],'Color',[0 0.4470 0.7410]);
hold off
...
legend(['Re $A(\xi)$'],['Im $A(\xi)$'],'Interpreter','latex','Location','southeast');
 f2 = gca;
    f2.FontSize = 16;
xticks([-120 0])
...
text(20,1.0,'$\chi(\xi)=-\mathrm{sign}(\xi)$','FontSize',14,'Interpreter','latex')
%text
ylim([-1.2,1.2])
xlim([-500,140])
ax = gca;
